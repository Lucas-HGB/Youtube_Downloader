...
class Filter:
	...
	def filter_artist(artist):
		...
		for pattern, replacement in Configs().artist_regex.items():
			logger.info(f'Replacing artist {replacement} with pattern {pattern} and value {artirts}')
	...
	def filter_title(title):
		...
		for pattern, replacement in Configs().title_regex.items():
			logger.info(f'Replacing title {replacement} with pattern {pattern} and value {artirts}')
